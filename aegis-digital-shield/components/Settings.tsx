          ...
          <button onClick={() => setShowDeactivate(true)} className="w-full bg-rose-600/10 border border-rose-500/40 text-rose-500 py-6 rounded-xl font-bold uppercase tracking-[0.2em] hover:bg-rose-600 hover:text-white transition-all">
            ...
            <div className="space-y-6">
              <input 
                ...
                className={`w-full bg-black border p-5 rounded-xl text-center font-bold outline-none transition-all ${
                  error ? 'border-rose-500 text-rose-500 animate-shake' : 'border-rose-900 text-rose-500'
                }`} 
                placeholder="ENTER MASTER PASSWORD" 
              />
              {error && <p className="text-rose-500 text-[10px] text-center uppercase font-bold">Invalid Authorization Key</p>}
